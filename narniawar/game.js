...
class MapRenderer {
    ...
    drawTerritoryRegion(territory, era, isSelected) {
        ...
        const regionSize = 50; // Reduced for 24 territories
...
class GameLoop {
    constructor() {
        ...
        this.specialAbilityUsedThisTurn = false;
    ...
    initializeTerritories() {
        // Create 24 territories positioned across the Narnia map with good spacing
        const canvas = document.getElementById('map-canvas');
        const w = canvas.parentElement.clientWidth || 800;
        const h = canvas.parentElement.clientHeight || 600;

        // 24 territories spread across the map to prevent overlap
        const territories = [
            // NORTHWEST (Lantern Waste area)
            new Territory(1, 'Lantern Waste', w * 0.15, h * 0.18, [2, 3, 4], 'FOREST'),
            new Territory(2, 'Western Woods', w * 0.18, h * 0.35, [1, 3, 16, 18], 'FOREST'),
            new Territory(3, 'The Lamp-post', w * 0.22, h * 0.28, [1, 2, 4, 18], 'FOREST'),
            new Territory(4, 'Witch\'s Castle', w * 0.35, h * 0.18, [1, 3, 5, 19], 'MOUNTAIN'),

            // NORTH (Ettinsmoor / Shribble area)
            new Territory(5, 'Ettinsmoor', w * 0.55, h * 0.12, [4, 6, 24], 'MOUNTAIN'),
            new Territory(6, 'River Shribble', w * 0.78, h * 0.12, [5, 7, 8], 'LAND'),

            // EAST COAST (Northern Marsh / Cair Paravel)
            new Territory(7, 'Northern Marsh', w * 0.82, h * 0.25, [6, 8, 9], 'COASTAL'),
            new Territory(8, 'Owlwood', w * 0.78, h * 0.35, [6, 7, 9, 24], 'FOREST'),
            new Territory(9, 'Cair Paravel', w * 0.88, h * 0.45, [7, 8, 10, 23], 'COASTAL'),
            new Territory(10, 'Galma', w * 0.95, h * 0.55, [9, 11], 'COASTAL'),

            // SOUTHEAST COAST
            new Territory(11, 'Glasswater Creek', w * 0.82, h * 0.65, [10, 12, 21], 'COASTAL'),
            new Territory(12, 'Stormness Head', w * 0.85, h * 0.80, [11, 13], 'COASTAL'),

            // SOUTH (Archenland / Mountains)
            new Territory(13, 'Anvard', w * 0.70, h * 0.85, [12, 14, 22], 'MOUNTAIN'),
            new Territory(14, 'Archenland', w * 0.55, h * 0.88, [13, 15, 22], 'MOUNTAIN'),
            new Territory(15, 'Mt. Pire', w * 0.38, h * 0.88, [14, 17], 'MOUNTAIN'),

            // WEST & CENTRAL
            new Territory(16, 'Cauldron Pool', w * 0.12, h * 0.52, [2, 17, 18], 'MOUNTAIN'),
            new Territory(17, 'Shuddering Wood', w * 0.40, h * 0.55, [15, 16, 19, 21, 22], 'FOREST'),
            new Territory(18, 'Beaversdam', w * 0.32, h * 0.35, [2, 3, 16, 19], 'LAND'),
            new Territory(19, 'Frozen Lake', w * 0.42, h * 0.40, [4, 18, 17, 21], 'LAND'),

            // CENTER (Aslan / Fords / Battlefield)
            new Territory(20, 'Fords of Beruna', w * 0.60, h * 0.52, [11, 17, 19, 21, 22, 23, 24], 'LAND'),
            new Territory(21, 'Stone Table', w * 0.72, h * 0.55, [11, 20, 22, 23], 'LAND'),
            new Territory(22, 'Dancing Lawn', w * 0.58, h * 0.65, [13, 14, 17, 20, 21], 'FOREST'),
            new Territory(23, 'Aslan\'s Camp', w * 0.68, h * 0.42, [9, 20, 21, 24], 'LAND'),
            new Territory(24, 'Battle Field', w * 0.65, h * 0.32, [5, 8, 20, 23], 'LAND')
        ];
        ...
        territories[12].owner = 2; // Anvard (Player 2)
        territories[12].addUnit(new Unit('FAUN', 2, 13));
        territories[12].addUnit(new Unit('FAUN', 2, 13));
        territories[12].addUnit(new Unit('FAUN', 2, 13));
    ...
    initializeUI() {
        ...
        // Auto-Claim button
        document.getElementById('btn-auto-claim').addEventListener('click', () => {
            this.autoClaimTerritories();
        });
    ...
    updateTerritoryPositions() {
        ...
        // 24 territories in a grid layout to prevent overlap
        const positions = [
            { x: 0.15, y: 0.18 }, // 0: Lantern Waste
            { x: 0.18, y: 0.35 }, // 1: Western Woods
            { x: 0.22, y: 0.28 }, // 2: Lamp-post
            { x: 0.35, y: 0.18 }, // 3: Witch's Castle
            { x: 0.55, y: 0.12 }, // 4: Ettinsmoor
            { x: 0.78, y: 0.12 }, // 5: Shribble
            { x: 0.82, y: 0.25 }, // 6: Northern Marsh
            { x: 0.78, y: 0.35 }, // 7: Owlwood
            { x: 0.88, y: 0.45 }, // 8: Cair Paravel
            { x: 0.95, y: 0.55 }, // 9: Galma
            { x: 0.82, y: 0.65 }, // 10: Glasswater Creek
            { x: 0.85, y: 0.80 }, // 11: Stormness Head
            { x: 0.70, y: 0.85 }, // 12: Anvard
            { x: 0.55, y: 0.88 }, // 13: Archenland
            { x: 0.38, y: 0.88 }, // 14: Mt. Pire
            { x: 0.12, y: 0.52 }, // 15: Cauldron Pool
            { x: 0.40, y: 0.55 }, // 16: Shuddering Wood
            { x: 0.32, y: 0.35 }, // 17: Beaversdam
            { x: 0.42, y: 0.40 }, // 18: Frozen Lake
            { x: 0.60, y: 0.52 }, // 19: Fords of Beruna
            { x: 0.72, y: 0.55 }, // 20: Stone Table
            { x: 0.58, y: 0.65 }, // 21: Dancing Lawn
            { x: 0.68, y: 0.42 }, // 22: Aslan's Camp
            { x: 0.65, y: 0.32 }  // 23: Battle Field
        ];
    ...
    showTutorial() {
        const tutorialSteps = [
            ...
            {
                title: '🎲 Combat (Risk Rules)',
                message: 'Combat is decided by dice. The attacker rolls up to 3 dice, the defender up to 2. We compare the highest rolls. Defender wins all ties! If you lose, you lose a unit.'
            },
            {
    ...
    updatePhaseUI() {
        ...
        const autoClaimBtn = document.getElementById('btn-auto-claim');

        // Always hide auto-claim by default
        autoClaimBtn.classList.add('hidden');
        ...
        switch (this.phase) {
            case 'CLAIMING':
                ...
                autoClaimBtn.classList.remove('hidden');
                break;
            case 'DEPLOYMENT':
    ...
    handleTerritoryClick(territory) {
        ...
        // Universal selection for special abilities
        ...
        if (this.phase === 'FORTIFY') {
            if (this.selectedTerritory && this.selectedTerritory !== territory) {
                ...
                if (territory.owner === this.currentPlayer && isAdjacent) {
                    ...
                    this.selectedTerritory = territory;
                    this.showNotification('Fortify Selected', `${territory.name} selected. Click an adjacent friendly territory to move units.`);
                    this.render();
                }
            } else if (territory.owner === this.currentPlayer && territory.units.length > 1) {
                this.selectedTerritory = territory;
    ...
    resolveCombat(attacker, defender) {
        ...
        for (let i = 0; i < Math.min(attackDice, defendDice); i++) {
            const atk = attackRolls[i];
            const def = defendRolls[i];
            if (atk > def) {
                defenderLosses++;
                breakdown.push(`🎲 ${atk} vs ${def} → ATK wins!`);
            } else {
                attackerLosses++;
                breakdown.push(`🎲 ${atk} vs ${def} → DEF wins!${atk === def ? ' (Tie)' : ''}`);
            }
        }
        ...
        setTimeout(() => {
            ...
            const battleSummary = `<div style="text-align: center; margin-bottom: 10px;">` +
                `<span style="color: var(--p1-color);"><strong>ATK</strong> [${attackRolls.join(', ')}]</span> vs ` +
                `<span style="color: var(--p2-color);"><strong>DEF</strong> [${defendRolls.join(', ')}]</span></div>` +
                `<hr>` +
                `<div style="font-size: 15px; background: rgba(0,0,0,0.05); padding: 10px; border-radius: 4px;">` +
                breakdown.join('<br>') +
                `</div>` +
                `<hr>` +
                `<div style="text-align: center; font-weight: bold;">TOTAL LOSSES</div>` +
                `<div style="display: flex; justify-content: space-around; margin-top: 5px;">` +
                `<div>Attacker: <span style="color:var(--p2-color);">-${attackerLosses}</span></div>` +
                `<div>Defender: <span style="color:var(--p1-color);">-${defenderLosses}</span></div></div>`;
            ...
            if (activeDefenders === 0) {
                ...
                this.showNotification(
                    '🛡️ DEFENSE HELD',
                    `${defender.name} survived the attack!\n\n${battleSummary}`
                );
            }
            this.render();
        }, 1500);
    ...
    useSpecialAbility() {
        ...
        if (this.specialAbilityUsedThisTurn) {
            this.showNotification('❌ Already Used', `You can only use your ${ability} ability once per turn!`);
            return;
        }

        switch (this.currentEra.id) {
            case 1: // Era 1: Plant - Add reinforcements to selected territory
                if (!this.selectedTerritory || this.selectedTerritory.owner !== this.currentPlayer) {
                    this.showNotification('⚠️ Select Territory', 'Select one of your territories to Plant reinforcements!');
                    return;
                }

                this.showNotification('🌱 Growth of Narnia', `The world blooms! +2 Fauns added to ${this.selectedTerritory.name}.`);
                this.selectedTerritory.addUnit(new Unit('FAUN', this.currentPlayer, this.selectedTerritory.id));
                this.selectedTerritory.addUnit(new Unit('FAUN', this.currentPlayer, this.selectedTerritory.id));
                this.specialAbilityUsedThisTurn = true;
                break;

            case 3: // Era 3: Raid - Move units extra
                if (!this.selectedTerritory || this.selectedTerritory.owner !== this.currentPlayer) {
                    this.showNotification('⚠️ Select Territory', 'Select a territory to launch a Cavalry Raid!');
                    return;
                }

                if (this.selectedTerritory.units.length < 2) {
                    this.showNotification('❌ Not Enough Units', 'Need at least 2 units to launch a raid!');
                    return;
                }

                this.showNotification('🏇 cavalry Raid', 'Quick maneuver! Move units extra distance (feature coming soon/placeholder).');
            ...
            case 5: // Era 5: Naval Supremacy - Patrol coastal areas
                this.showNotification('⛵ Naval Patrol', 'Reinforcements reach our shores! +1 unit added to all your coastal territories.');

                let coastalAdded = 0;
                this.territories.forEach(t => {
                    if (t.owner === this.currentPlayer && t.type === 'COASTAL') {
                        t.addUnit(new Unit('FAUN', this.currentPlayer, t.id));
                        coastalAdded++;
                    }
                });

                if (coastalAdded === 0) {
                    this.showNotification('❌ No Port', 'You don\'t control any coastal territories!');
                } else {
                    this.specialAbilityUsedThisTurn = true;
                }
                break;

            case 6: // Era 6: Enchantment - Convert enemy unit
                this.showNotification('Green Witch\'s Enchantment', 'Mental mastery! You exert control over the enemy (feature coming soon/placeholder).');
                this.specialAbilityUsedThisTurn = true;
                break;
    ...
    autoClaimTerritories() {
        if (this.state !== 'CLAIMING' && this.phase !== 'CLAIMING') return;

        // Shuffle unclaimed territories
        const unclaimed = [...this.unclaimedTerritories].sort(() => Math.random() - 0.5);

        // Assign them all to current and other player and move to next state
        unclaimed.forEach((territory) => {
            territory.changeOwner(this.currentPlayer);
            territory.addUnit(new Unit('FAUN', this.currentPlayer, territory.id));

            // Switch player for next assignment
            this.currentPlayer = this.currentPlayer === 1 ? 2 : 1;
        });

        this.unclaimedTerritories = [];
        this.state = 'PLAYING';
        this.phase = 'DEPLOYMENT';
        this.calculateReinforcements();

        this.showNotification('🎖️ Auto-Claim Complete!', 'All territories have been claimed. Now begins the age of conquest. Place your reinforcements!');
    ...
    showEraTransition() {
        ...
        // Manual dismissal for era transition
        const dismissTransition = () => {
            transition.removeEventListener('click', dismissTransition);
        ...
        transition.addEventListener('click', dismissTransition);

        // Add a prompt to the transition screen
        ...
        const existingPrompt = transition.querySelector('.transition-prompt');
        if (existingPrompt) existingPrompt.remove();

        transition.querySelector('.era-content').appendChild(prompt);
    }
    ...
    applyEraTheme() {
        ...
        // Calculate and set topbar text contrast
        const eraId = this.currentEra.id;
        let topbarText = 'var(--parchment)';
        if (eraId === 2 || eraId === 3) {
            topbarText = '#2c1810'; // Dark text for Winter/Golden eras
        }
        root.style.setProperty('--current-topbar-text', topbarText);
    ...
    showNotification(title, message, onOk = null) {
    ...
    showGameRules() {
        this.showNotification(
            ...
            '<p><strong>OBJECTIVE</strong><br>Conquer all territories or eliminate your opponent.</p>' +
            '<hr>' +
            '<p><strong>THE 3 PHASES OF BATTLE</strong></p>' +
            '<ul>' +
            '<li><strong>1. DEPLOYMENT:</strong> Receive reinforcements based on your holdings. Click your territories to place units.</li>' +
            '<li><strong>2. ATTACK:</strong> Select a territory with 2+ units, then click an adjacent enemy to strike.</li>' +
            '<li><strong>3. FORTIFY:</strong> Move units between adjacent friendly territories to secure your borders.</li>' +
            '</ul>' +
            '<hr>' +
            '<p><strong>COMBAT MECHANICS (DICE)</strong></p>' +
            '<ul>' +
            '<li>Attacker rolls up to <strong>3 dice</strong>.</li>' +
            '<li>Defender rolls up to <strong>2 dice</strong>.</li>' +
            '<li>We compare the highest rolls. <strong>Ties ALWAYS go to the Defender!</strong></li>' +
            '</ul>' +
            '<hr>' +
            '<p><strong>THE ETERNAL CYCLE</strong><br>Every few turns, an <strong>Era</strong> changes the world with unique rules and powerful Special Abilities.</p>' +
            '</div>'
        );
    }
}
